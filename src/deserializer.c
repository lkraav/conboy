		       ...
		       GError              **error)
{
  ...
  GtkTextTag *tag = NULL;
	...
	case STATE_FORMAT:
	case STATE_CONTENT:
		/* TODO: Make it more generic. ATM only italic and bold are supported */
		/* TODO: Maybe put to extra function. */
		...
		/*
		if (ELEMENT_IS("bold")) {
			tag = gtk_text_tag_table_lookup(info->buffer->tag_table, "bold");
			if (tag == NULL) {
				tag = gtk_text_buffer_create_tag(info->buffer, "bold", "weight", PANGO_WEIGHT_BOLD, NULL);
			}
		} else if (ELEMENT_IS("italic")) {
			tag = gtk_text_tag_table_lookup(info->buffer->tag_table, "italic");
			if (tag == NULL) {
				tag = gtk_text_buffer_create_tag(info->buffer, "italic", "style", PANGO_STYLE_ITALIC, NULL);
			}
		} else {
			g_printerr("ERRRRRRORRRR: Can only be a <italic> or <bold> tag. But is <%s>.", element_name);
		}
		*/
		     ...
		     GError              **error)
{
  ...
  case STATE_FORMAT:
	  ...
	  /*
	  if (ELEMENT_IS)("bold") || ELEMENT_IS("italic")) { //This is not generic enough
		  pop_state(info);
		  g_assert(peek_state(info) == STATE_FORMAT ||
				   peek_state(info) == STATE_CONTENT);
		  info->tag_stack = g_slist_delete_link(info->tag_stack, info->tag_stack);
	  } else {
		  g_printerr("ERRRROR: Expecting </bold> or </italic>");
	  }
	  */
	  break;
